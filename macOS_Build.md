...
Vì một số lý do mà XXKey không thể đưa lêp Mac App Store được, nếu các bạn không yên tâm về file build chính thức từ tác giả, các bạn có thể tải mã nguồn về tự build lấy XXKey cho mình. 

Yêu cầu:
- macOS Mojave trở lên.
- XCode 10 trở lên.

Tải mã nguồn từ dự án XXKey trên GitHub về, mở dự án XXKey bằng cách vào:
...
Giờ bạn vào menu Product -> Archive để Xcode tiến hành build XXKey ở dạng Production (có thể dùng được), sau khi build xong hộp thoại sẽ xuất hiện, bấm vào Distribute App
...
Bấm Next để chọn nơi lưu trữ file Build. Vậy là xong, bạn đã có ứng dụng XXKey.app của riêng bạn.
...
Chúc các bạn Build XXKey cho macOS thành công!
