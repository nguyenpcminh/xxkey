...
int main() {
    ...
    // "quy" (qu + y): marks go on y
    newSession();
    typeText("quyf");
    check(screen == L"quỳ", "quyf -> quỳ");
    newSession();
    typeText("quys");
    check(screen == L"quý", "quys -> quý");

    // "qu" + e + n: mark on e
    newSession();
    typeText("quenf");
    check(screen == L"quèn", "quenf -> quèn");

    // "toa" + s: mark on a ("toá")
    newSession();
    typeText("toas");
    check(screen == L"toá", "toas -> toá");

    // "biếc" style: iê + c final
    newSession();
    typeText("bieecs");
